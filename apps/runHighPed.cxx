...
int main(int argn, char** argc) {
  ...
  // configure
  AcdJobConfig jc("runHighPed.exe","This utility makes pedestal files from input digi files");
  ...
  // build filler 
  AcdCalibLoop_Digi r(AcdCalibData::PED_HIGH,jc.digiChain(),jc.optval_P(),jc.config());

  // run!
  r.go(jc.optval_n(),jc.optval_s());    

  // do fits
  AcdPedestalFitLibrary pedFitter(AcdPedestalFitLibrary::MeanValue,kTRUE);
  AcdCalibMap* peds = r.fit(pedFitter,AcdCalibData::PED_HIGH,AcdCalib::H_RAW,jc.refFileName().c_str());
